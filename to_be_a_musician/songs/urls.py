...
urlpatterns = patterns('',
    ...
    url(r"{0}/interpretation/(?P<uuid>[\w_-]+)/$".format(SONG_URL_PATTERN),
        InterpretationView.as_view(), name='songs_interpretation_detail'),
    url(r"{0}/interpretation/(?P<uuid>[\w_-]+)/edit/$".format(SONG_URL_PATTERN),
        interpretation_update_view, name='songs_interpretation_edit'),
    url(r"{0}/interpretation/(?P<uuid>[\w_-]+)/delete/$".format(SONG_URL_PATTERN),
